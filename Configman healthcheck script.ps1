...
.DESCRIPTION
    ...
    It compiles the results into a single, styled HTML file for easy review.
    The script must be run on a machine with the Configuration Manager console installed, typically the Primary Site Server.
.NOTES
    Author: Gemini
    Version: 1.0
    Prerequisites:
...
try {
    ...
    Write-Error "Failed to connect to SCCM site $SiteCode. Please ensure the Configuration Manager console is installed and the site code is correct."
    # Create a basic error report
    $head = @"
    <style>body { font-family: sans-serif; } h1 { color: red; }</style>
    <h1>Error: Failed to connect to SCCM Site $SiteCode</h1>
    <p>Please ensure the Configuration Manager console is installed and the script is run from a machine with access.</p>
...
# 1. Server Connectivity and Performance
$reportFragments += "<h2>1. Server Connectivity and Performance</h2>"
$tableRows = @()
try {
...
# 2. Sites and Hierarchy
$reportFragments += "<h2>2. Sites and Hierarchy</h2>"
$tableRows = @()
...
# 3. SQL Server
$reportFragments += "<h2>3. SQL Server</h2>"
$tableRows = @()
$sqlServerName = $Site.SQLServerName
try {
    $sqlService = Get-Service -Name "MSSQL`$$($Site.SQLServerInstanceName)" -ComputerName $sqlServerName
    $sqlAgentService = Get-Service -Name "SQLAGENT`$$($Site.SQLServerInstanceName)" -ComputerName $sqlServerName
...
# 4. Maintenance Tasks
$reportFragments += "<h2>4. Maintenance Tasks</h2>"
$tableRows = @()
$maintTasks = Get-CMSiteMaintenanceTask
$failedTasks = $maintTasks | Where-Object { $_.LastStatus -ne "Completed" }
if ($failedTasks) {
    $tableRows += New-HealthCheckRow -CheckItem "Overall Maintenance Task Status" -Status "Warning" -Notes "$($failedTasks.Count) task(s) have not completed successfully."
...
# 5. Management Point
$reportFragments += "<h2>5. Management Point (MP)</h2>"
$tableRows = @()
...
# 6. Collection
$reportFragments += "<h2>6. Collection</h2>"
$tableRows = @()
...
# 7. Distribution Point (DP)
$reportFragments += "<h2>7. Distribution Point (DP)</h2>"
$tableRows = @()
...
# 8. Software Updates (SUP)
$reportFragments += "<h2>8. Software Updates (SUP)</h2>"
$tableRows = @()
try {
    $sup = Get-CMSoftwareUpdatePoint
    $syncStatus = Get-CMSoftwareUpdateSynchronizationStatus
    $tableRows += New-HealthCheckRow -CheckItem "WSUS Synchronization Status" -Status "Healthy" -Notes "Last Sync: $($syncStatus.LastSyncTime), Status: $($syncStatus.LastSyncStateName)"
...
# 9. Devices
$reportFragments += "<h2>9. Devices</h2>"
$tableRows = @()
$totalDevices = (Get-CMDevice | Measure-Object).Count
$activeDevices = (Get-CMDevice | Where-Object { $_.IsActive -eq $true } | Measure-Object).Count
$activePercent = [math]::Round(($activeDevices / $totalDevices) * 100, 2)
$status = if ($activePercent -lt 90) { "Warning" } else { "Healthy" }
